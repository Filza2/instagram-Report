        ...
        try:
            ...
            if 'No users found' in r2.text:
                try:
                    ...
                    if 'The link you followed may be broken, or the page may have been removed' in adv_search.text:
                    ...
                    elif 'profile_id' in adv_search.text:
                        target_id=re.findall('"profile_id":"(.*?)"',adv_search.text)[0]
                        Report_Instagram(target_id,sessionid,csrftoken)
                    elif 'page_id' in adv_search.text:
                        target_id=re.findall('"page_id":"profilePage_(.*?)"',adv_search.text)
                        Report_Instagram(target_id,sessionid,csrftoken)
                    else:
                        console.print('\n- [bold red]Failed[/bold red] to get target username, Try entering the Target ID manually .\n')
                        target_id=input('- Enter The Target ID : ')
                    ...
                    console.print(f'- [bold red]Error[/bold red], {e}');exit()
            else:
